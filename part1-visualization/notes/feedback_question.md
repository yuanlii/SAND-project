...
__08/20/2019__:
Problem:
- can get response from elasticsearch db, but fail to serialize as json files, but with html format --> How to solve this problem?


__08/28/2019__:
- IP vs host names mapping --> IP is fixed or keep changing?
- some of the host names are shown as IP? Are they important? --> get unique hosts of hundreds, remember only 200+?
- ps_track: for each intermediate hops (are the names for the hops available?)

ideally, want to get a table with fields `host name`, `IP`, `coordinates` 

for ps_trace, care about the entire path (unique identifier as hashing) or details about the hops (e.g., max rtt)?
